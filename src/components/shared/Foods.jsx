      ...
      <div className={style.foods} key={foods.name}>
        ...
        <div className={style.cartFoods}>
          ...
          <div className={style.buttonContainer}>
            {quantityCount(state, foods.id) === 1 && (
              <button
                className={style.smallButton}
                onClick={() => dispatch(removeItem(foods))}
              >
            ...
            {quantityCount(state, foods.id) > 1 && (
              <button
                className={style.smallButton}
                onClick={() => dispatch(decrease(foods))}
              >
                <img src={minus} alt="minus_icon" />
              </button>
            )}
            {quantityCount(state, foods.id) > 0 && (
              <span className={style.counter}>
                {quantityCount(state, foods.id)}
              </span>
            )}
            {isInCart(state, foods.id) ? (
              <button
                className={style.smallButton}
                onClick={() => dispatch(increase(foods))}
              >
